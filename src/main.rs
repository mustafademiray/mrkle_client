...
impl MerkleNode {
    ...
    fn get_parent(&self) -> Option<&MerkleNode> {
        self.parent.as_deref()
    }

    fn get_left(&self) -> Option<&MerkleNode> {
        self.left.as_deref()
    }

    fn get_right(&self) -> Option<&MerkleNode> {
        self.right.as_deref()
    }
}
...
impl MerkleTree {
    ...
    fn generate_merkle_tree(data: &[String]) -> Option<Box<MerkleNode>> {
        ...
        //println!("Leaf nodes are: {:?}", leaf_nodes);
        ...
        //println!("Generated Nodes After Loop: {:?}", nodes);
...
fn generate_merkle_proof(merkle_tree: &MerkleTree, index_to_verify: usize) -> Option<Vec<(String, String)>>
{
    ...
    while index > 0
    {
        ...
        let sibling_node = if let Some(parent) = current.get_parent()
        {
            if is_left_child {
                parent.get_right()
            }
        ...
        match sibling_node
        {
            Some(node) =>
            {
                proof.push((if is_left_child { "right" } else { "left" }, node.hash.clone()));
            }
            None => return None,
        }

        current = if let Some(parent) = current.get_parent()
        {
            parent
        }
        else
...
fn verify_hash_using_proof(merkle_proof: &[(String, String)], hash_to_verify: &str) -> bool {
    ...
    let mut position = "left";  // Initialize the position as "left".

    println!("We want to verify this: {:?}", hash_to_verify);
    for (proof_position, sibling_hash) in merkle_proof {
        ...
        if proof_position == "left" {
            ...
            position = "right"; // Reverse the position
        } else {
            hash = hash_data(&format!("{}{}", hash, sibling_hash));
            position = "left"; // Reverse the position
        }
